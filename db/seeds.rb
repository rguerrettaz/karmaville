...
fields = [:first_name, :last_name, :email, :username, :created_at, :updated_at, :karma_points_sum]
TOTAL_USERS.times.each_slice(SLICE_SIZE).each_with_index do |ids, index|
  data = ids.map do |i|
    [Faker::Name.first_name, Faker::Name.last_name, "email_#{i}@example.com", "user_#{i}", now, now, 0]
  end
